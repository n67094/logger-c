...
static char *LoggerGetTime()
{
  ...
  if (currentDate == (time_t)(-1)) {
    date = malloc(sizeof(char) * strlen(LOGGER_NO_TIME_PREFIX) + 1);
...
static int LoggerCloseFile() { return fclose(file); }

static void LoggerInFile(
    int msgLength, const char *date, const char *prefix, const char *msg, va_list args)
{
  /* + 1 for \n added below*/
    ...
    va_list args)
{
  ...
  *fullMsg                = '\0';
  ...
  if (LOGGER_WITH_COLOR) strcat(fullMsg, LOGGER_FAINT);

  strcat(fullMsg, date);

  if (LOGGER_WITH_COLOR) strcat(fullMsg, LOGGER_RESET_STYLE);

  if (LOGGER_WITH_COLOR) strcat(fullMsg, color);
...
/*
 * Since va_copy is only available in c99 I use two va_list, va_list are invalidated by
 * vprintf and vfprintf so it cannot be used twice
 */
static void Logger(
    const char *color,
    const char *msg,
    ...
    va_list vList2)
{
  ...
  if (LOGGER_IN_CONSOLE) {
    ...
    if (LOGGER_WITH_COLOR == 1) {
      ...
      fullMsgLength = faintStyleLength + dateLength + resetStyleLength + colorLength +
                      prefixLength + resetStyleLength + msgLength;
    } else {
...
void LoggerSetLevel(int flags) { loggerLevel = flags; }
