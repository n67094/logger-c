...
#include <time.h>
  ...
  static void LoggerInit() UNUSED_ATTRIBUTE;
  static int LoggerClose() UNUSED_ATTRIBUTE;

  static void LoggerSetLevel(int flags) UNUSED_ATTRIBUTE;
  static void LoggerInfo(const char *msg, ...) UNUSED_ATTRIBUTE;
  static void LoggerDebug(const char *msg, ...) UNUSED_ATTRIBUTE;
  static void LoggerSuccess(const char *msg, ...) UNUSED_ATTRIBUTE;
  static void LoggerWarning(const char *masg, ...) UNUSED_ATTRIBUTE;
  static void LoggerError(const char *msg, ...) UNUSED_ATTRIBUTE;
      ...
      va_list args)
  {
    ...
    *full_msg      = '\0';
    ...
    if (LOGGER_WITH_COLOR) strcat(full_msg, LOGGER_RESET_STYLE);

    if (LOGGER_WITH_COLOR) strcat(full_msg, color);
      ...
      va_list v_list_2)
  {
    ...
    if (LOGGER_IN_CONSOLE)
    {
      ...
      else { full_msg_length = date_length + prefix_length + msg_length; }
  ...
  void LoggerInit()
  {
    if (LOGGER_IN_FILE) { LoggerOpenFile(); }
