...
#include <time.h>
  ...
  static char *LoggerGetTime()
  {
    ...
    if (current_date == (time_t)(-1)) {
      date =
          (char *)malloc(sizeof(char) * strlen(LOGGER_NO_TIME_PREFIX) + 1);
      ...
      strftime(
          date, date_length, "%Y-%m-%d %H:%M:%S", gmtime(&current_date));
      ...
      va_list v_list_2)
  {
    ...
    if (LOGGER_IN_CONSOLE) {
      ...
      if (LOGGER_WITH_COLOR == 1) {
        ...
        full_msg_length = faint_style_length + date_length +
                          reset_style_length + color_length +
                          prefix_length + reset_style_length + msg_length;
      } else {
  ...
  void LoggerWarning(const char *msg, ...)
  {
    if (logger_level & LOGGER_LEVEL_WARNING) {
      ...
      Logger(
          LOGGER_YELLOW, msg, LOGGER_WARNING_PREFIX, v_list_1, v_list_2);
